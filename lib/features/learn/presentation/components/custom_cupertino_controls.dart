...
class _CustomCupertinoControlsState extends State<CustomCupertinoControls>
  ...
  Widget _buildHitArea() {
    ...
    // Hide the center play button completely
    final bool showPlayButton = false;

    return GestureDetector(
      onTap: () {
        if (_latestValue.isPlaying) {
          // Video is playing - toggle controls visibility
          _hideTimer?.cancel();
          setState(() {
            notifier.hideStuff = !notifier.hideStuff;
          });

          // If we just showed the controls, start the hide timer
          if (!notifier.hideStuff) {
            _startHideTimer();
          }
        } else {
          // Video is paused - toggle controls visibility
          _hideTimer?.cancel();
          setState(() {
            notifier.hideStuff = !notifier.hideStuff;
          });
